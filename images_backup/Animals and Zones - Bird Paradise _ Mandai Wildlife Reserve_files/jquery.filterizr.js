...
(function(global, $) {
    ...
    */
    $.fn.filterizr.prototype = {
        ...
        */
        search: function(text, isPrefix=false) {
            var self   = this,
                //get active category
            ...
            if (self._isSearchActivated()) {
                for (i = 0; i < array.length; i++) {
                    ...
                    var containsText = array[i].data('sort').toLowerCase().trim().indexOf(text.toLowerCase()) > -1;
                    if (isPrefix) {
                        containsText = array[i].data('sort').toLowerCase().trim().startsWith(text.toLowerCase());
                    }
                    if (containsText) {
                        target.push(array[i]);
                    }
        ...
        dropdown: function(text, isPrefix=false) {
            var self   = this,
                //get active category
                array  = self._multifilterModeOn() ?
                            self._makeMultifilterArray() :
                ...
                self._typedText = '';
                self._isPrefix = isPrefix;
                self.dropdownFilter = text;

            for (i = 0; i < array.length; i++) {
                var containsText = array[i].find('.btn-animal-primary').hasClass(text);
        ...
        */
        sort: function(attr, sortOrder) {
            ...
            if (self._isSearchActivated())
                self.search(self._typedText, self._isPrefix);
            else
            // self._placeItems(target);
            self.dropdown(self.dropdownFilter);
        },
        ...
        */
        _setupControls: function() {
            ...
            $('*[data-nice-select]').change(function() {
                self._delayEvent(function() {
                    self.dropdown(this.value);
                }, 250, self._uID);
            });
        },

...
try {
    ...
    try {
      ...
      for (link of document.getElementsByTagName('link')) {
        if (
          ...
          link.getAttribute('href').includes('.staah.')
        ) {
          allow = false;
        }
      }
      ...
      if (pixel_details.type == 'a') {
        allow = false;
        if (
          new Set(['882', '62160', '998', '68204', '1552']).has(
            pixel_details.pixel_id
          )
        ) {
          allow = true;
        }
      }
      if (url.includes('confined=true')) {
        allow = false;
      }
      if (
        (pixel_details.type == 's' ||
          pixel_details.type == 'c' ||
          pixel_details.type == 'a') &&
        allow
      ) {
        if (pixel_details.type) {
          ...
          if (checkAndFireUrl(firing_url)) {
            if (pixel_details.type == 'c') {
              sjrn_fc_sdk(pixel_details.pixel_id, all_params, 'container');
            } else if (pixel_details.type == 'a') {
              sjrn_fc_sdk(pixel_details.pixel_id, all_params, 'advertiser');
            } else {
  ...
  function sjrn_fc_sdk(pixel_id, all_params, pixel_type) {
    var sdk_params = {};
    ...
    sjn('initAndFire', pixel_type, pixel_id, eventType, {
      context: {
        vertical: 'hotel',
      },
      params: sdk_params,
  ...
  function sjrn_su(url) {
    ...
    if (url.includes('static.sojern.com/cip/c/')) {
      ...
      type = 's';
    } else if (url.includes('static.sojern.com/cip/a/')) {
      pixel_id = url.split('static.sojern.com/cip/a/')[1].split('.')[0];
      type = 'a';
    }
